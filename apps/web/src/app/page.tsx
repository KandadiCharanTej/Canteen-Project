  ...
  return (
    <div className="bg-[#f8f9fa] min-h-screen pb-24 md:pb-8">
      ...
      <header className="bg-white sticky top-0 z-40 px-4 py-3 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] md:hidden flex justify-between items-center">
        ...
        {user ? (
          <button onClick={() => router.push("/profile")} className="w-9 h-9 rounded-full bg-primary/10 text-primary font-bold text-sm flex items-center justify-center">
            {user.name.charAt(0)}
          </button>
        ) : (
          <button onClick={() => router.push("/login")} className="text-xs font-bold text-primary bg-primary/10 px-3 py-1.5 rounded-lg">
            Login
          </button>
        )}
      </header>
