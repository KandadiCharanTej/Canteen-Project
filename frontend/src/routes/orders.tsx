        ...
        <div className="mt-3 inline-flex p-0.5 rounded-xl bg-muted text-[12px] font-medium">
          {(["active", "previous"] as const).map((t) => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                "h-8 px-4 rounded-lg transition",
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        highlight ? "border-primary shadow-[var(--shadow-pop)]" : "border-border",
      )}
    >
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      >
        <div
          className={cn(
            "h-10 w-10 rounded-lg grid place-items-center text-base shrink-0",
            ...
            <div className="px-3 pb-3 border-t border-border pt-3 space-y-3">
                ...
                {STATUSES.map((s, i) => {
                  ...
                  return (
                    <div key={s} className="flex-1 flex flex-col items-center">
                      <div
                        className={cn("h-2 w-2 rounded-full", done ? "bg-primary" : "bg-border")}
                      />
                      <span
                        className={cn(
                      ...
                      {i < STATUSES.length - 1 && (
                        <div
                          className={cn(
                            "h-[2px] w-full -mt-3 -z-10",
                            done && i < stepIdx ? "bg-primary" : "bg-border",
                          )}
              ...
              <div
                className={cn(
                  ...
                  isReady ? "bg-success/10 border border-success/30" : "bg-muted",
                )}
              >
                <KeyRound
                  className={cn("h-5 w-5", isReady ? "text-success" : "text-muted-foreground")}
                />
