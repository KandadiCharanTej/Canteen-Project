...
function ProfilePage() {
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-md px-4 pt-10 text-center">
          <div className="h-16 w-16 mx-auto rounded-2xl gradient-primary grid place-items-center text-white text-xl font-bold">
            Q
          </div>
          <h2 className="mt-3 text-base font-semibold">Sign in to QuickBite</h2>
          <p className="text-[12px] text-muted-foreground mt-1">
            Track your orders, save favourites, and pay faster.
          </p>
          <Link
            to="/login"
            search={{ next: "/profile" }}
            className="mt-4 inline-flex h-10 px-5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold"
          >
            Sign in
          </Link>
        </div>
  ...
  orders.forEach((o) =>
    o.items.forEach((i) => (counts[i.food.name] = (counts[i.food.name] || 0) + i.qty)),
  );
  const favs = Object.entries(counts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3);
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-md px-4 pt-4">
        ...
        >
          <div className="h-16 w-16 mx-auto rounded-full gradient-primary grid place-items-center text-white text-lg font-bold">
            {initials}
          </div>
        ...
        <div className="mt-3 bg-card border border-border rounded-2xl p-3">
          ...
          {favs.length ? (
            ...
            <p className="text-[12px] text-muted-foreground">
              Order something to see your favourites.
            </p>
          )}
        </div>
        ...
        <Link
          to="/admin/login"
          className="mt-3 flex items-center gap-2 px-3 h-11 bg-card border border-border rounded-xl text-sm"
        >
        ...
        <button
          onClick={logout}
          className="mt-2 w-full h-11 rounded-xl bg-destructive/10 text-destructive font-semibold text-sm inline-flex items-center justify-center gap-2"
        >
