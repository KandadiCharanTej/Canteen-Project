      ...
      <div className="flex-1 grid place-items-center px-4 py-6">
        ...
        >
          <div className="flex flex-col items-center mb-4">
            <div className="h-12 w-12 rounded-2xl gradient-primary grid place-items-center text-white text-lg font-bold shadow-[var(--shadow-pop)]">Q</div>
          ...
          {step === "info" ? (
            <form onSubmit={sendOtp} className="space-y-2.5">
              ...
              <Field icon={<Phone className="h-4 w-4" />} placeholder="10-digit phone" value={phone} onChange={(v) => setPhone(v.replace(/\D/g, "").slice(0, 10))} type="tel" />
              ...
              <Field icon={<GraduationCap className="h-4 w-4" />} placeholder="Department / Class" value={department} onChange={setDepartment} />
            ...
            <div className="space-y-3">
              <p className="text-[12px] text-muted-foreground text-center">Enter the OTP sent to +91 {phone}</p>
              <input
              ...
              />
              <button onClick={verify} className="w-full h-11 rounded-xl bg-primary text-primary-foreground font-semibold text-sm">Verify & Continue</button>
              <button onClick={() => setStep("info")} className="w-full text-[12px] text-muted-foreground hover:text-foreground">Edit details</button>
            </div>
          )}

          <div className="mt-4 pt-3 border-t border-border text-center">
            <Link to="/admin/login" className="text-[11px] text-muted-foreground hover:text-primary inline-flex items-center gap-1">
