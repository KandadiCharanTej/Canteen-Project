      ...
      <div className="flex-1 grid place-items-center px-4">
        <motion.form onSubmit={submit} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex flex-col items-center mb-4">
            <div className="h-12 w-12 rounded-2xl bg-primary/15 grid place-items-center text-primary"><ShieldCheck className="h-6 w-6" /></div>
            <h1 className="mt-2 text-base font-semibold">Canteen Staff Login</h1>
            <p className="text-[12px] text-muted-foreground">Hint: password is <code className="font-mono bg-muted px-1 rounded">admin123</code></p>
          </div>
          <label className="flex items-center gap-2 h-11 px-3 rounded-xl bg-muted">
            ...
            <input type="password" autoFocus value={pwd} onChange={(e) => setPwd(e.target.value)}
              placeholder="Staff password" className="flex-1 bg-transparent outline-none text-sm" />
          </label>
          <button className="mt-3 w-full h-11 rounded-xl bg-primary text-primary-foreground font-semibold text-sm">Sign in</button>
        </motion.form>
      </div>
    </div>
  );
