...
function PaymentPage() {
  ...
  const complete = () => {
    ...
    const order = placeOrder({
      items: cart, total: grand, pickupTime: pickup, instructions, paid: true,
    ...
    return (
      <div className="min-h-screen grid place-items-center p-6 text-center">
        <div>
          ...
          <button onClick={() => navigate({ to: "/" })} className="mt-3 h-10 px-4 rounded-xl bg-primary text-primary-foreground text-sm font-semibold">Browse menu</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="px-4 h-14 flex items-center gap-2 border-b border-border/60 sticky top-0 bg-background/85 glass-blur z-10">
        <button onClick={() => navigate({ to: "/cart" })} className="p-2 -ml-2 rounded-full hover:bg-muted">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <span className="font-semibold">Payment</span>
        <span className="ml-auto text-[12px] text-muted-foreground">Pickup: <span className="font-semibold text-foreground">{pickup}</span></span>
      </header>
      ...
      <div className="flex-1 mx-auto w-full max-w-md px-4 py-4">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Amount payable</div>
          ...
          <div className="mt-4 mx-auto h-40 w-40 rounded-xl bg-white border-2 border-dashed border-border grid place-items-center relative overflow-hidden">
            ...
            <div className="absolute inset-0 grid place-items-center">
              <div className="h-10 w-10 rounded-lg bg-primary grid place-items-center text-white font-bold text-xs shadow-md">QB</div>
            </div>
          </div>

          <button onClick={copy} className="mt-3 inline-flex items-center gap-1.5 h-9 px-3 rounded-lg bg-muted hover:bg-accent/50 text-sm font-medium">
            {copied ? <Check className="h-3.5 w-3.5 text-success" /> : <Copy className="h-3.5 w-3.5" />}
            {UPI}
          </button>
          <p className="text-[11px] text-muted-foreground mt-1.5">Scan QR or pay using any UPI app below</p>
        </motion.div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          {apps.map((a) => (
            ...
            >
              <div className="h-9 w-9 rounded-lg grid place-items-center text-lg" style={{ background: a.color + "15" }}>{a.emoji}</div>
        ...
        <button onClick={complete} className="mt-4 w-full h-12 rounded-xl bg-success text-success-foreground font-semibold text-sm active:scale-[0.99] transition">
          ✓ I Have Completed Payment
        </button>
        <p className="text-[10px] text-center text-muted-foreground mt-2">Order will move to “Payment Checking” for staff verification.</p>
      </div>
        ...
        {opening && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.85 }} animate={{ scale: 1 }}
              ...
              <p className="mt-3 text-sm font-semibold">Opening {apps.find((a) => a.id === opening)?.name}…</p>
              <p className="text-[11px] text-muted-foreground mt-1">Complete payment in the app, then return here.</p>
            </motion.div>
          </motion.div>
        )}
        {completed && (
...
function QrPattern() {
  ...
  const cells = Array.from({ length: 144 }, (_, i) => Math.random() > 0.55 || [0, 1, 2, 9, 10, 11, 132, 133, 134].includes(i));
  return (
