...
import { Clock, KeyRound, MessageSquare, Phone, CheckCircle2, ChefHat, BellRing, IndianRupee } from "lucide-react";
...
const STATUSES: { key: Order["status"]; label: string; color: string }[] = [
  { key: "Payment Checking", label: "Payment", color: "bg-warning/15 text-warning-foreground border-warning/30" },
    ...
    <div className="mx-auto max-w-5xl px-4 py-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">{orders.length} total · refresh as new orders arrive</p>
        </div>
      </div>

      {/* Status counters */}
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        <button onClick={() => setFilter("All")} className={cn("p-2.5 rounded-xl border text-left transition", filter === "All" ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/40")}>
          <div className="text-[10px] uppercase font-semibold text-muted-foreground">All</div>
          <div className="text-xl font-bold">{orders.length}</div>
        </button>
        {STATUSES.map((s) => (
          <button key={s.key} onClick={() => setFilter(s.key)} className={cn("p-2.5 rounded-xl border text-left transition", filter === s.key ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/40")}>
      ...
      <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {visible.length === 0 && (
          <div className="col-span-full text-center text-sm text-muted-foreground py-12">
            No orders to show. <span className="block text-[11px] mt-1">When students order, they'll appear here.</span>
          </div>
        )}
        <AnimatePresence>
          {visible.map((o) => (
            <AdminOrderCard key={o.id} order={o}
              onAction={(next) => {
                updateOrder(o.id, { status: next, paid: next === "Payment Checking" ? o.paid : true });
...
function AdminOrderCard({ order, onAction }: { order: Order; onAction: (next: Order["status"]) => void }) {
  const next: Record<Order["status"], { label: string; status: Order["status"]; icon: typeof IndianRupee } | null> = {
    ...
    <motion.div layout
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="text-[11px] text-muted-foreground inline-flex items-center gap-1"><Phone className="h-3 w-3" /> {order.customerPhone}</div>
        </div>
        <span className={cn("text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded-md border", STATUSES.find((s) => s.key === order.status)?.color)}>
          {order.status}
        </span>
      </div>

      <div className="flex items-center gap-3 text-[12px]">
        <span className="inline-flex items-center gap-1 font-semibold"><Clock className="h-3.5 w-3.5 text-primary" /> {order.pickupTime}</span>
      ...
      <div className="flex items-center gap-2 bg-primary/5 border border-primary/20 rounded-lg px-2.5 py-1.5">
        ...
        <div className="flex-1">
          <div className="text-[9px] uppercase tracking-wider text-muted-foreground">Pickup OTP</div>
          <div className="text-base font-bold tracking-[0.25em] text-primary leading-none">{order.otp}</div>
        </div>
      </div>

      {action && (
        <button onClick={() => onAction(action.status)}
          ...
          {action.status === "Preparing" ? <ChefHat className="h-4 w-4" /> : <action.icon className="h-4 w-4" />}
          {action.label}
        </button>
      )}
    </motion.div>
